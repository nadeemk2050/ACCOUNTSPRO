...
const PackagingSmartReportModal = ({ 
        ...
        const bagUnsubs = uidCandidates.flatMap(uid => {
            return ['userId', 'ownerId', 'companyId'].map(field => {
                ...
                return onSnapshot(q, (snap) => {
                    ...
                    setBags([...merged.values()]);
                }, (err) => console.warn(`Bags sync error (${field}):`, err));
            });
        });

        // 3. Fallback: Load recent bags regardless of UID if count is low (Handles legacy/unsynced data)
        const fallbackUnsub = onSnapshot(query(collection(db, 'jumbo_bags'), limit(5000)), (snap) => {
            bagCache.current['fallback'] = snap.docs.map(d => ({ id: d.id, ...d.data() }));
            const merged = new Map();
            Object.values(bagCache.current).forEach(list => list.forEach(b => merged.set(b.id, b)));
            setBags([...merged.values()]);
        });
    ...
    // Dynamic Self-Healing to remove historically orphaned manufacturing fills of reusable jumbo bags
    React.useEffect(() => {
        if (!isOpen || !targetUid || reusableBags.length === 0) return;

        const healOrphanedHistory = async () => {
            try {
                for (const rb of reusableBags) {
                    if (Array.isArray(rb.usageHistory) && rb.usageHistory.length > 0) {
                        let needsUpdate = false;
                        const nextHistory = [];

                        for (const h of rb.usageHistory) {
                        ...
                        if (needsUpdate) {
                            ...
                            console.log('Self-healed reusable bag #' + rb.bagNo + ' - removed orphaned manufacturing entries.');
                        }
                    }
                }
            } catch (err) {
        ...
        healOrphanedHistory();
    }, [isOpen, targetUid, reusableBags.length]);
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                                    {viewingDetail ? viewingDetail.replace('_', ' ') : 'Traceability & Analysis Dashboard'}
                                </p>
                            </div>
                        </div>
                        ...
                        {!viewingDetail ? (
                            <div className="w-full h-full flex flex-col p-10">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-6">
                                    {/* Button 5: NEW - Jumbo Bags Allocated Inward */}
                                    <button 
                                        onClick={() => setViewingDetail('jumbo_allocated')}
                                        className="group relative bg-white rounded-[2rem] p-8 border border-slate-200 shadow-xl hover:shadow-2xl hover:border-violet-500/50 transition-all duration-500 flex flex-col items-center text-center overflow-hidden h-full"
                                    >
                                        <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-right from-violet-500 to-purple-600 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                                        <div className="w-16 h-16 rounded-2xl bg-violet-50 text-violet-600 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-500 shadow-inner">
                                            <Box size={32} />
                                        </div>
                                        <h3 className="text-base font-black text-[#1e3264] uppercase tracking-tight mb-3 leading-tight">
                                            Jumbo Bags Allocated Inward
                                        </h3>
                                        <div className="flex flex-col gap-1 mb-6">
                                            <div className="flex items-center justify-center gap-2">
                                                <span className="text-[10px] font-black text-violet-600 bg-violet-50 px-2 py-0.5 rounded-full border border-violet-100">
                                                    {(() => {
                                                        const allocated = stockJournals.flatMap(sj => getBagsForVoucher(sj));
                                                ...
                                                <span className="text-[10px] font-black text-[#1e3264] bg-slate-100 px-2 py-0.5 rounded-full border border-slate-200">
                                                    {(() => {
                                                        const refs = new Set(
                                                            stockJournals
                                                                .map(sj => String(sj.refNo || '').trim().toUpperCase())
                                                                .filter(Boolean)
                                                        );
                                                        return refs.size;
                                                    })()} REF NOS
                                                ...
                                                <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
                                                    {formatWeight(stockJournals.flatMap(sj => getBagsForVoucher(sj)).reduce((acc, b) => acc + Number(b.qty || 0), 0))} KG
                                                </span>
                                            </div>
                                            <p className="text-[10px] text-slate-500 font-medium leading-relaxed">
                                                Audit of all bags allocated in manufacturing. Cross-referenced with Production Registers.
                                            </p>
                                        </div>
                                        <div className="mt-auto px-4 py-2 bg-violet-50 text-violet-700 text-[9px] font-black rounded-full opacity-0 group-hover:opacity-100 transition-opacity uppercase">
                                            View Allocation Audit
                                        </div>
                                    </button>
                                        ...
                                        <div className="flex flex-col gap-1 mb-6">
                                            <div className="flex items-center justify-center gap-2">
                                                <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">
                                                    {(() => {
                                                        const inBags = stockJournals.flatMap(sj => getBagsForVoucher(sj));
                                                        return inBags.length;
                                                    })()} BAGS
                                                </span>
                                                <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
                                                    {formatWeight(stockJournals.reduce((acc, sj) => acc + getBagsForVoucher(sj).reduce((s, b) => s + Number(b.qty || 0), 0), 0))} KG
                                                </span>
                                            </div>
                                            <p className="text-[10px] text-slate-500 font-medium leading-relaxed">
                                        ...
                                        <div className="flex flex-col gap-1 mb-6">
                                            <div className="flex items-center justify-center gap-2">
                                                ...
                                                <span className="text-[10px] font-black text-blue-700 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">
                                                    {stockJournals.reduce((acc, sj) => acc + getBagsForVoucher(sj).length, 0)} BAGS
                                                </span>
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col gap-1 mb-6">
                                            <div className="flex items-center justify-center gap-2">
                                                <span className="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-100">
                                                    {bags.filter(b => b.status === 'sold' && b.soldDate >= dateRange.from && b.soldDate <= dateRange.to).length} BAGS
                                                </span>
                                            </div>
                                            <p className="text-[10px] text-slate-500 font-medium leading-relaxed">
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col gap-1 mb-6">
                                            ...
                                            <p className="text-[10px] text-slate-500 font-medium leading-relaxed">
                                                Bags approved for multiple refills across production cycles.
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex flex-col gap-1 mb-6">
                                            <div className="flex items-center justify-center gap-2">
                                                <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded-full border border-blue-100">
                                                     {(() => {
                                                         const inBags = stockJournals.flatMap(sj => getBagsForVoucher(sj));
                                                         const activeBags = inBags.filter(b => {
                                                             const dbBag = bags.find(gb => gb.id === b.id);
                                                             const isSold = b.status === 'sold' || (dbBag && dbBag.status === 'sold');
                                                             return !isSold;
                                                         });
                                                         const activeReusableCount = (reusableBags || []).filter(rb => rb.status === 'active').length;
                                                         return Math.max(0, activeBags.length - activeReusableCount);
                                                     })()} BAGS
                                                </span>
                                            </div>
                                            <p className="text-[10px] text-slate-500 font-medium leading-relaxed">
                            ...
                            const directManufactured = bags
                                .filter(isManufacturedBag)
                                .map((b) => ({
                                    ...b,
                                    stockJournalRefNo: b.stockJournalRefNo || sjRefById.get(String(b.stockJournalId || b.linkedStockJournalId || '')) || '',
                                    voucherRefNo: b.voucherRefNo || b.stockJournalRefNo || sjRefById.get(String(b.stockJournalId || b.linkedStockJournalId || '')) || ''
                                }));
                            ...
                            const productionBagsList = allHistoricalBags.filter((b) => {
                                const bagDate = normalizeDateKey(b.date);
                            ...
                            const filteredBagsList = subTabFilteredList
                                ...
                                .sort((a, b) => {
                                    ...
                                    if (sortConfig.key === 'date') {
                                        ...
                                        aVal = (a.stockJournalRefNo || a.voucherRefNo || '').toLowerCase();
                                        bVal = (b.stockJournalRefNo || b.voucherRefNo || '').toLowerCase();
                            ...
                            const confirmDeleteOrphanBag = async () => {
                                if (deletePassword === "abcd") {
                                    try {
                                        ...
                                        setDeletePassword('');
                                        // No annoying alert on success, it updates reactively.
                                    } catch (err) {
                            ...
                            const SortIcon = ({ column }) => {
                             ...
                             const getSummary = () => {
                                 ...
                                 // Logic: OUT Balance = totalIn - remaining
                                 Object.keys(summaryMap).forEach(pId => {
                                     summaryMap[pId].totalOut = summaryMap[pId].totalIn - summaryMap[pId].remaining;
                                                        ...
                                                        {viewMode === 'detail' ? (
                                                            <>
                                                                <th 
                                                                    ...
                                                                    onClick={() => requestSort('bagNo')}
                                                                >
                                                                    <div className="text-white mb-1 text-[26px] font-bold leading-tight tracking-tighter">{filteredBagsList.filter(b => !b.isRefill).length}</div>
                                                            ...
                                                            <tr key={`${bag.id}-${i}`} className={`h-[21px] ${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} hover:bg-blue-50 transition-colors group`}>
                                                                ...
                                                                <td className="px-6 py-0 leading-none">
                                                                    <div className="flex items-center gap-2">
                                                                        ...
                                                                        {bag.isRefill ? (
                                                                            <span className="bg-amber-100 text-amber-800 px-1.5 py-0.5 rounded text-[8px] font-black uppercase border border-amber-200 ml-1">
                                                                                REFILL #{bag.refillCount}
                                                                            </span>
                                                                        ) : (
                                                                            (bag.isReusable || bag.reusableBagId || reusableBags.some(rb => String(rb.bagNo || '').replace(/^#/, '').trim().toUpperCase() === String(bag.bagNo || '').replace(/^#/, '').trim().toUpperCase())) && (
                                                                ...
                                                                <td className="px-6 py-0 font-bold text-[#1e3264] text-[10px] leading-none uppercase">
                                                                    {getBagRefNo(bag) === '-' && readyStockSubTab === 'in' ? (
                                                                        <div className="flex items-center justify-between w-full">
                                                                            ...
                                                                            <button 
                                                                                onClick={(e) => { e.stopPropagation(); handleDeleteOrphanBag(bag); }} 
                            ...
                            const sortedJournals = [...stockJournals]
                                ...
                                .sort((a, b) => {
                                    ...
                                    if (sortConfig.key === 'date') {
                                        ...
                                        aVal = getBagsForVoucher(a).length;
                                        bVal = getBagsForVoucher(b).length;
                                    } else if (sortConfig.key === 'weight') {
                            ...
                            const totalBagsProduced = sortedJournals.reduce((acc, sj) => acc + getBagsForVoucher(sj).length, 0);
                                            ...
                                            <table className="w-full text-left border-collapse">
                                                ...
                                                <tbody className="divide-y divide-slate-100 text-xs">
                                                    {sortedJournals.map((sj, idx) => {
                                                        const producedWt = (sj.produced || []).reduce((sum, p) => sum + Number(p.quantity || 0), 0);
                                                        const mfgBags = getBagsForVoucher(sj);
                                    ...
                                    <div className="flex items-center justify-between mb-2">
                                        <div>
                                            ...
                                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">
                                                Bags approved for multiple refills — select in manufacturing allocation
                                            </p>
                                        </div>
                                        <button
                                            ...
                                            <table className="w-full text-left border-collapse">
                                                <thead>
                                                    <tr className="bg-teal-800 text-white">
                                                        ...
                                                        <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest">Allowed to Refill in Jumbo Bag Allocation?</th>
                                    ...
                                    <div className="bg-gradient-to-r from-teal-500 to-cyan-600 px-6 py-4 flex items-center justify-between text-white">
                                        <div>
                                            <div className="text-white font-black uppercase text-sm tracking-tight">
                                                Refillable Bag Details: #{selectedReusableBag.bagNo}
                                            </div>
                                        ...
                                        <table className="w-full text-left border-collapse">
                                            ...
                                            <tbody className="divide-y divide-slate-100">
                                                ...
                                                {(selectedReusableBag.usageHistory || []).length === 0 && (
                                                    <tr>
                                                        <td colSpan="6" className="px-6 py-20 text-center text-slate-400 font-bold uppercase tracking-widest italic">
                                                            No refill history yet for this reusable bag.
                                                        </td>
                                                    </tr>
                                                )}
                                            </tbody>
