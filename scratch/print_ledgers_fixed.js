...
async function run() {
    ...
    const getLedger = (partyId, partyLabel) => {
        console.log(`\n=================== FIXED LEDGER FOR: ${partyLabel} (${partyId}) ===================`);
        ...
        invoices.forEach(inv => {
            ...
            if (isMainParty) {
                ...
                const drName = 'Purchase A/c';
                const crName = inv.partyName || 'Supplier';
                const particulars = `${drName} / ${crName}`;
                console.log(`  -> Row type: PURCHASE INV (Supplier), amount: ${amt}, particulars: "${particulars}"`);
            }
            ...
            if (hasAddlSplit) {
                ...
                if (matchesPartyLedger) {
                    const drName = 'Purchase Cost';
                    const crName = 'IZHAR AHMAD CO KHAWAR'; // resolved
                    const particulars = `${drName} / ${crName}`;
                    console.log(`  -> Row type: PURCHASE EXP [Path 2], amount: ${addlExpBase}, particulars: "${particulars}"`);
                }
            }
        });
    };
